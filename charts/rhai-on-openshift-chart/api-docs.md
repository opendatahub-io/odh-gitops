...
| components.aigateway.modelsAsAService.gateway.create | bool | `false` | Flag to create the Gateway (auto, true, false). If auto, created when modelsAsAService is Managed. |
| components.aigateway.modelsAsAService.gatewayClass | object | `{"create":false,"name":"maas-gateway-class","spec":{"controllerName":"openshift.io/gateway-controller/v1"}}` | GatewayClass configuration for the LLMInferenceService. |
| components.aigateway.modelsAsAService.gatewayClass.create | bool | `false` | Flag to create the GatewayClass (auto, true, false). If auto, created when modelsAsAService is Managed. |
