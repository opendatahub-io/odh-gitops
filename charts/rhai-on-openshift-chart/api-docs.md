...
| components.aigateway.modelsAsAService.gateway.spec.listeners[0].allowedRoutes.namespaces | string | `nil` | REQUIRED: set `from` to Selector (recommended) or Same. Template will fail if not set. |
...
| components.kserve.gateway.spec.listeners[0].allowedRoutes.namespaces | string | `nil` | REQUIRED: set `from` to Selector (recommended) or Same. Template will fail if not set. |
