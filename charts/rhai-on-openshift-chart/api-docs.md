...
| components.kserve.dependencies.jobSet | string | `nil` | JobSet dependency. Null uses profile default (true for default, false for inference). |
...
| profile | string | `"default"` | Deploy profile: sets default managementState for components and services. Options: default (all Removed), inference (KServe only) Explicit managementState values override the profile. |
