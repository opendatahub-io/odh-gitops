...
- **Deploy profiles**: Preconfigured deployment types (e.g., `inference`) that enable the right components and dependencies with a single flag
...
# Install inference-only stack (KServe + required dependencies)
...
| `inference` | Inference/model-serving stack | kserve | certManager, leaderWorkerSet, rhcl |
...
- Profiles also control component dependency defaults (e.g., `inference` disables `jobSet` for kserve since it's not needed for inference-only deployments)

### Example: Inference-only deployment
