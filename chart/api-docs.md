...
| components.kserve.modelsAsService.gateway.create | string | `"auto"` | Flag to create the Gateway (auto, true, false). If auto, it will be created if Models as Service is Managed. |
| components.kserve.modelsAsService.gatewayClass | object | `{"create":"auto","name":"maas-gateway-class","spec":{"controllerName":"openshift.io/gateway-controller/v1"}}` | Gateway Class configuration for the LLMInferenceService. |
| components.kserve.modelsAsService.gatewayClass.create | string | `"auto"` | Flag to create the Gateway Class (auto, true, false). If auto, it will be created if Models as Service is Managed. |
